...
def sim_data(n: int = 100,
             intercept: float = -5,
             two_way_coef: float = None,
             linear_vars: int = 10,
             linear_var_coef: float = None,
             noise_vars: int = 0,
             corr_vars: int = 0,
             corr_type: str = "AR1",
             corr_value: float = 0,
    ...
    """
    Simulate data for classification or regression that includes an interaction between two linear features, and some
    non-linear and linear features. Noise variables, correlated variables that are not predictive and surrogate features
    which are just derived from features that are predictive are also added.

    This function is for the most part a direct translation of the twoClassSim function from the R package caret - the
    option for an ordinal outcome and binary outcome mis-labelling were omitted. Full credit for the approach used for
    simulating binary classification data goes to the Authors and contributors of caret (Caret: Kuhn, M. (2008). Caret
    package. Journal of Statistical Software, 28(5). https://rdrr.io/cran/caret/man/twoClassSim.html)
    ...
    3. The addition of a linear feature that is a copy of another used in the linear predictor with a small amount
    of noise has been added to allow for the study of variable surrogacy/redundancy
    ...
    :param two_way_coef: list of three coefficients: two linear terms and an interaction effect
    :param linear_vars: number of linear features
    :param linear_var_coef: an optional list of coefficients for linear features if the default is not desired
    :param noise_vars: number of unrelated independent noise features (do not contribute to the linear predictor)
    :param corr_vars: number of unrelated correlated noise features (do not contribute to the linear predictor)
    ...
    # add two correlated normal features for use in creating an interaction term in the linear predictor
    ...
    lp = intercept - two_way_coef[0] * tmp_data.TwoFactor1 + two_way_coef[1] * tmp_data.TwoFactor2 \
    ...
    if linear_vars > 0:
        if linear_var_coef is None:
            lin_coeff = np.linspace(linear_vars, 1, num=linear_vars) / 4
            ...
            if linear_vars != len(linear_var_coef):
                raise ValueError(
                    f"User defined linear feature coefficient list must be of length {linear_vars}"
                )
